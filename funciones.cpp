...
bool  caminar_f() {
  analisis_alta(ax, nuevo_caminarx, viejo_caminarx, axMEDIA, superior_caminarx, diferencia_caminarx, pre_decre_caminarx, pre_incre_caminarx, pre_a_caminarx, contador_caminarx, fin_caminarx , time_caminarx, time_caminar_finalx,time_valido_x)
  analisis_alta(ay, nuevo_caminary, viejo_caminary, ayMEDIA, superior_caminary, diferencia_caminary, pre_decre_caminary, pre_incre_caminary, pre_a_caminary, contador_caminary, fin_caminary , time_caminary, time_caminar_finaly,time_valido_y)
  analisis_alta(az, nuevo_caminarz, viejo_caminarz, azMEDIA, superior_caminarz, diferencia_caminarz, pre_decre_caminarz, pre_incre_caminarz, pre_a_caminarz, contador_caminarz, fin_caminarz, time_caminarz, time_caminar_finalz,time_valido_x)
  ...
  if (fin_caminarx or fin_caminary or fin_caminarz) {
    ...
    if ((time_camiarx > minimo_caminal and time_camiarx < maximo_caminar ) or (time_camiary > minimo_caminar and time_camiary < maximo_caminal ) or (time_camiarz > minimo_caminar and time_camiarz < maximo_caminar )) {
      return 1;
  ...
  bool correr_f() {
    if (fin_caminarx or fin_caminary or fin_caminarz) {
      if ((time_camiarx > minimo_correr and time_camiarx < maximo_correr ) or (time_camiary > minimo_correr and time_camiary < maximo_correr ) or (time_camiarz > minimo_correr and time_camiarz < maximo_correr )) {
        return 1;
